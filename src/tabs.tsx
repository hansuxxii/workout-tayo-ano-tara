...
import { PLACEHOLDER_GIF, flowOrder, timerPresets, workoutFlows } from "./workoutFlows";
...
export function Card({
  ...
  return <div className={`rounded-3xl border-4 border-black bg-white ${className}`}>{children}</div>;
}
...
export function Button({
  ...
  const base =
    "inline-flex items-center justify-center rounded-2xl border-2 border-black px-4 py-3 text-sm font-medium transition";
  const style =
    variant === "outline"
      ? "bg-white text-black"
      : "bg-black text-white";
...
export function PixelCard({
  ...
  return (
    <Card className={`shadow-[8px_8px_0_0_rgba(0,0,0,1)] ${className}`}>
      {children}
    </Card>
  );
}
